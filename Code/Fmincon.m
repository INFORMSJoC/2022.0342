...
%
%% Read in CSV File
% SCATSEV the lower the better symptons, higher the worse/more symptons
sex = 'M';
prev_Concussion = "3";

input_SCAT = csvread(strcat("Data/SCATSEV_category_", sex, "_Hx", prev_Concussion, ".csv"),1,1); %offset row 1 col 1
input_SAC = csvread(strcat("Data/SAC_category_", sex, "_Hx", prev_Concussion, ".csv"),1,1); %offset row 1 col 1
input_SCAT(:,2) = input_SCAT(:,2) + 1; %observation discrete states start with 1 to 6 here.
input_SAC(:,2) = input_SAC(:,2) + 1;

input = input_SAC;
SCATSEV_M = containers.Map; % create dictionary, each key is 1 observation sequence
...
for idx = 1:length(unique_ID) % loop through unique ID, store each obs to dict
    ...
    SCATSEV_M(string(unique_ID(idx))) = current_portion;
end

outfolder_SCAT = strcat("./EstimatedParam_SCATSEV_", sex, "_Hx", prev_Concussion);
outfolder_SAC = strcat("./EstimatedParam_SAC_", sex, "_Hx", prev_Concussion);
...
for EM_iter = 1:Total_EM_iters
    ...
    for i_ObsSeq = 1:length(unique_ID)
        ...
        observed = SCATSEV_M(string(unique_ID(i_ObsSeq)));
        T = length(observed);
    ...
    baron_A = [Fmincon_Aineq_Trans; Fmincon_Aeq_Trans];
    baron_rl = [zeros((N)*(N-1),1); Fmincon_beq_Trans];
    baron_ru = [Fmincon_bineq_Trans; Fmincon_beq_Trans]; 
    [transition_fmincon,fval,ef,info] = baron(@(P)Likelihood_Func_H(P,H_fmincon),baron_A,baron_rl,baron_ru,Fmincon_lb_Trans,Fmincon_ub_Trans);
    ...
    tic
    emission_fmincon = fmincon(@(P)Likelihood_Func_L(P, L_fmincon, SCATSEV_M, unique_ID, O),emission_fmincon,Fmincon_Aineq_Emis,Fmincon_bineq_Emis,Fmincon_Aeq_Emis,Fmincon_beq_Emis,Fmincon_lb_Emis,Fmincon_ub_Emis);
    total_time_fmincon = total_time_fmincon + toc;
...
% aic = -2*log(aic/Total_MCMCIter) + 2*(N^2 + O*N - 1);
